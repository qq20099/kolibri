...
<div class="img-form-wrapper text-center">
    <div class="img-wrapper <?=$model_name?> mb-2">
        <img src="<?=((!$model->img) ? '../images/no-img.png': $path.$model->img)?>" alt="" />
    </div>
    <label <?if(!$model->img):?><?else:?>style="display:none"<?endif?> class="btn btn-success">
        <input id="upload-img" type="file" name="myFiles" hidden />
        Додати зображення
    </label>
    <button type="button" class="btn btn-warning" id="delete-img" <?if(!$model->img):?>style="display:none"<?endif?>>Видалити зображення</button>
    <div hidden>
